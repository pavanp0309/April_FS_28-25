...
let srinvas=data[0].brand
console.log(srinvas)
usersearch=document.getElementById("search")
function searchfn(){
  console.log(usersearch.value)
   let filterdata=data.filter(e=>e.title.toLowerCase().includes(usersearch.value))
   console.log(filterdata)
}



let filteredpro=data.filter(ele=>ele.price<2)
// let sortpro=filteredpro.sort((a,b)=>a-b)


let products=data.map(ele=>{
    const {thumbnail,title}=ele
    ...
    return  `   <div class="col-sm-12 col-md-3 col-lg-3">
            <div class="card" >
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">${ele.price.toFixed(0)}</p>
                    <a href="#" class="btn btn-primary">addtocart</a>
                </div>
            </div>
         </div>`
})
let prod=document.getElementById("products").innerHTML=products
let cartTotal=data.reduce((t,e)=>{
    console.log("t",t)
    console.log(e?.price)
    return t+e?.price
},0)
